 ...
 */
package br.jus.trt8.pjecalc.negocio.dominio.calculo.salariofamilia.regras;
...
public class PeriodoSalarioFamiliaValidRule
implements ValidRule {
    ...
    private Date sugerirDataFim(Calculo calculo) {
        if (Utils.naoNulo(calculo.getDataDemissao()) && Utils.naoNulo(calculo.getDataTerminoCalculo())) {
            if (HelperDate.getInstance(calculo.getDataDemissao()).greaterThen(calculo.getDataTerminoCalculo())) {
                HelperDate terminoCalculo = HelperDate.getInstance(calculo.getDataTerminoCalculo());
                if (terminoCalculo.getDay() != terminoCalculo.daysInMonth()) {
                    terminoCalculo.setDay(1);
                    terminoCalculo.addMonth(-1);
                }
                return terminoCalculo.getDate();
            }
    ...
    public Periodo getPeriodoSugerido(SalarioFamilia salarioFamilia) {
        Calculo calculo = salarioFamilia.getCalculo();
    ...
    public boolean isValid(ValidValueValidator validator, ValidatorContext context) {
        ...
        SalarioFamilia salarioFamilia = (SalarioFamilia)context.getBean();
        Periodo periodoSugerido = this.getPeriodoSugerido(salarioFamilia);
        if (!periodoSugerido.isCompleto()) {
            return true;
        }
        return validator.getFlag() == 1 ? HelperDate.getCurrentCompetence((Date)value).lessThanOrEqualsTo(HelperDate.getCurrentCompetence(periodoSugerido.getFinal())) : HelperDate.getCurrentCompetence((Date)value).greaterThenOrEquals(HelperDate.getCurrentCompetence(periodoSugerido.getInicial()));
    }
}
