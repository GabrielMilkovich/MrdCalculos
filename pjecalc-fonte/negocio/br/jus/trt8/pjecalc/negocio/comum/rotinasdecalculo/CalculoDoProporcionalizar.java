...
public class CalculoDoProporcionalizar
extends RotinaDeCalculo {
    ...
    public CalculoDoProporcionalizar(Periodo periodo, BigDecimal valor) {
        this.setEntrada(PERIODO, periodo);
        this.setEntrada(VALOR, valor);
        this.setEntrada(QTD_EXCLUSOES, 0);
    }

    public CalculoDoProporcionalizar(Periodo periodo, BigDecimal valor, Integer qtdExclusoes) {
        this.setEntrada(PERIODO, periodo);
        this.setEntrada(VALOR, valor);
    ...
    public void executar() {
        ...
        switch (HelperDate.getInstance(periodo.getFinal()).daysInMonth()) {
            case 28: {
                resultado = new BigDecimal(diasDoPeriodo).divide(new BigDecimal(28), Utils.CONTEXTO_MATEMATICO).multiply(valor, Utils.CONTEXTO_MATEMATICO);
                break;
            }
            case 29: {
                resultado = new BigDecimal(diasDoPeriodo).divide(new BigDecimal(29), Utils.CONTEXTO_MATEMATICO).multiply(valor, Utils.CONTEXTO_MATEMATICO);
                break;
            }
            default: {
                resultado = new BigDecimal(diasDoPeriodo).divide(new BigDecimal(30), Utils.CONTEXTO_MATEMATICO).multiply(valor, Utils.CONTEXTO_MATEMATICO);
            }
        }
