/**
 * PJe-Calc v2.15.1 — IndiceIPC
 * Porte 1:1 de: br.jus.trt8.pjecalc.negocio.dominio.indices.ipc.IndiceIPC
 */
...
import { TABELA_IPC } from './tabela-ipc';

export class IndiceIPC extends IndiceBase {
  constructor(competencia: Date, taxa: Decimal, dataCriacao?: Date) {
    super(competencia, taxa, dataCriacao);
  }

  static obterTabela(periodo: Periodo): IndiceIPC[] {
    const lista: IndiceIPC[] = [];
    ...
    for (const entry of TABELA_IPC) {
      const comp = new Date(entry.ano, entry.mes - 1, 1);
      if (comp < inicio.getDate() || comp > fim.getDate()) continue;
      lista.push(new IndiceIPC(comp, new Decimal(entry.taxa)));
    }
    lista.sort((a, b) => a.getCompetencia().getTime() - b.getCompetencia().getTime());
    return calcularIndiceAcumulado(lista) as IndiceIPC[];
  }

  clonar(): IndiceIPC {
    const c = new IndiceIPC(this.competencia, this.taxa, this.dataCriacao);
