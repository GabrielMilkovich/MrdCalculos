/**
 * =====================================================
 * MODULO: ADICIONAL DE TRANSFERENCIA (Art. 469 §3 CLT)
 * =====================================================
 * 25% transfer premium when employee is transferred provisionally.
 * Monthly: salary x 25%
 */
...
export class AdicionalTransferenciaModule implements VerbaModule {
  readonly id = 'ADIC_TRANSFERENCIA';
  readonly nome = 'Adicional de Transferencia';
  ...
  resolveInputs(ctx: VerbaModuleContext, verba: PjeVerba): ResolvedInputs {
    ...
    return {
      ...
      multiplicador: verba.multiplicador || 0.25,
    };
  }
  ...
  buildAuditTrail(ctx: VerbaModuleContext, inputs: ResolvedInputs, resultado: number): ModuleAuditEntry[] {
    return [
      { campo: 'base', valor: inputs.base, fonte: inputs.baseSource, regra: 'Art. 469 §3 CLT' },
      { campo: 'multiplicador', valor: inputs.multiplicador, fonte: 'config_verba (25%)', observacao: 'Transferencia provisoria' },
      { campo: 'resultado', valor: resultado, fonte: 'formula:BxMxQ/D' },
    ];
  }
}

registerVerbaModule(new AdicionalTransferenciaModule());
